...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_NPC_VENDOR_REF, "SELECT item, maxcount, incrtime, ExtendedCost, group_id FROM npc_vendor WHERE entry = ? ORDER BY slot ASC", CONNECTION_SYNCH);
